...
rm -fr "${PROJECT_ROOT}/pkg/legacy/clientset"

versions=(v1alpha1 v1beta1 v1beta2 v1beta3 v1beta4 v1beta5)

# Prefix ${REPOSITORY}/pkg/legacy/apis/direct.csi.min.io/ to each versions.
arr=("${versions[@]/#/$REPOSITORY/pkg/legacy/apis/direct.csi.min.io/}")
...
deepcopy-gen \
    ...
    --output-package "${REPOSITORY}/pkg/legacy/"

for version in "${versions[@]}"; do
    repo="${REPOSITORY}/pkg/legacy/apis/direct.csi.min.io/${version}"
...
client-gen \
    ...
    --input-base "${REPOSITORY}/pkg/legacy/apis"
...
conversion-gen \
    --go-header-file "${SCRIPT_ROOT}/boilerplate.go.txt" \
    --input-dirs "${versions[-1]/#/$REPOSITORY/pkg/legacy/apis/direct.csi.min.io/}" \
    --output-package "${REPOSITORY}/pkg/legacy/" 

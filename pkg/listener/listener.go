...
var errLeaderElectionDied = errors.New("leader election died")
...
func (listener *Listener) Run(ctx context.Context) error {
	...
	eventBroadcaster.StartRecordingToSink(
		&corev1.EventSinkImpl{
			Interface: k8s.KubeClient().CoreV1().Events(namespace),
		},
	)

	leader := k8s.SanitizeResourceName(fmt.Sprintf("%s/%s", listener.leaderLock, listener.identity))
	resourceLock, err := resourcelock.New(
		resourcelock.LeasesResourceLock,
		namespace,
		leader,
		k8s.KubeClient().CoreV1(),
		k8s.KubeClient().CoordinationV1(),
		resourcelock.ResourceLockConfig{
			Identity: k8s.SanitizeResourceName(id),
			EventRecorder: eventBroadcaster.NewRecorder(scheme.Scheme, apicorev1.EventSource{
				Component: leader,
			}),
		},
	)
	...
	leaderConfig := leaderelection.LeaderElectionConfig{
		...
		Callbacks: leaderelection.LeaderCallbacks{
			OnStartedLeading: func(ctx context.Context) {
				klog.V(2).Infof("Started %v controller after successful leader election", listener.handler.Name())

...
func (handler *testEventHandler) ListerWatcher() cache.ListerWatcher {
	return &cache.ListWatch{
		ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
			return handler.directCSIClient.DirectV1beta5().DirectCSIVolumes().List(context.TODO(), options)
		},
		WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
			return handler.directCSIClient.DirectV1beta5().DirectCSIVolumes().Watch(context.TODO(), options)
		},
	}
}
...
func TestListener(t *testing.T) {
	...
	testHandler.handleFunc = handleFunc
	for _, volume := range volumes {
		_, err := testHandler.directCSIClient.DirectV1beta5().DirectCSIVolumes().Update(
			ctx,
			volume,

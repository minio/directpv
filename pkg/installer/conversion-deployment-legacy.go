...
func deleteLegacyConversionSecret(ctx context.Context, identity string) error {
	return k8s.KubeClient().CoreV1().Secrets(k8s.SanitizeResourceName(identity)).Delete(ctx, conversionWebhookSecretName, metav1.DeleteOptions{})
}

func deleteLegacyConversionWebhookCertsSecret(ctx context.Context, identity string) error {
	return k8s.KubeClient().CoreV1().Secrets(k8s.SanitizeResourceName(identity)).Delete(ctx, conversionWebhookCertsSecret, metav1.DeleteOptions{})
}

...
func TestNodeGetVolumeStats(t *testing.T) {
			...
			Status: directcsi.DirectCSIVolumeStatus{
				...
				Conditions: []metav1.Condition{
					...
					},
					{
						...
						Reason:             string(directcsi.DirectCSIVolumeReasonAbnormal),
						LastTransitionTime: metav1.Now(),
					},
				},
			},
	...
	nodeServer.probeMounts = func() (map[string][]mount.Info, error) {
		return map[string][]mount.Info{
			"0:0": {
				{
					MountPoint: "/unknown/mnt",
				},
			},
		}, nil
	}
	...
	for i, testCase := range testCases {
		...
		if response.GetVolumeCondition().GetAbnormal() {
			volObj, gErr := nodeServer.directcsiClient.DirectV1beta3().DirectCSIVolumes().Get(ctx, testCase.request.GetVolumeId(), metav1.GetOptions{
				TypeMeta: utils.DirectCSIVolumeTypeMeta(),
			})
			if gErr != nil {
				t.Fatalf("case %v: volume (%s) not found. Error: %v", i+1, testCase.request.GetVolumeId(), gErr)
			}
			if !utils.IsCondition(volObj.Status.Conditions, string(directcsi.DirectCSIVolumeConditionAbnormal), metav1.ConditionTrue, string(directcsi.DirectCSIVolumeReasonAbnormal), response.GetVolumeCondition().GetMessage()) {

...
func getDrivesByIds(ctx context.Context, ids []string) <-chan utils.ListDriveResult {
	resultCh := make(chan utils.ListDriveResult)
	go func() {
		...
		for _, id := range ids {
			...
			resultCh <- utils.ListDriveResult{Drive: *d}
		}
	}()
	return resultCh
}
